...
class Tools {
    ...
    fun displayImageBusiness(ctx: Context?, img: ImageView?, url: String) {
        if (img != null) {
            Glide.with(ctx!!)
    ...
    fun setNavigationBarColor(act: Activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            val window = act.window
            window.navigationBarColor = act.resources.getColor(R.color.colorOnPrimary)
        }
    }
    ...
    fun setSystemBarColor(act: Activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            window.statusBarColor = act.resources.getColor(R.color.colorPrimary)
        }
    }
